...
SMODS.Joker {
    ...
    loc_vars = function (self, info_queue, card)
        return { vars = { BOINGULAR_PI[card.ability.extra.pi_digit] } }
    end,
    cost = 3,
    eternal_compat = true,
    ...
    calculate = function (self, card, context)
        if context.individual and context.cardarea == G.play then
            local ret = {
                mult = BOINGULAR_PI[card.ability.extra.pi_digit],
                colour = G.C.MULT
            }
            if not context.blueprint then
                card.ability.extra.pi_digit = card.ability.extra.pi_digit + 1
            end
            return ret
        end
    end
