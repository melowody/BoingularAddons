...
SMODS.Joker {
    ...
    loc_txt = {
        ...
        text = {
            "Played cards give",
            "{C:chips}+[next two digits of e] Chips{} when scored",
            "{C:inactive}(Currently {C:chips}+#1#{C:inactive} and {C:chips}+#2#{C:inactive} Chips){}"
        }
    },
    rarity = 1,
    ...
    loc_vars = function (self, info_queue, card)
        return { vars = { BOINGULAR_E[card.ability.extra.e_digit], BOINGULAR_E[card.ability.extra.e_digit+1] } }
    end,
    cost = 3,
    eternal_compat = true,
    ...
    calculate = function (self, card, context)
        if context.individual and context.cardarea == G.play then
            local ret = {
                chips = BOINGULAR_E[card.ability.extra.e_digit] + BOINGULAR_E[card.ability.extra.e_digit+1],
                colour = G.C.CHIPS
            }
            if not context.blueprint then
                card.ability.extra.e_digit = card.ability.extra.e_digit + 1
            end
            return ret
        end
    end
